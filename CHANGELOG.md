...
- WordPress Popular Posts: se usa `defer` v�a `script_loader_tag` (recomendaci�n del autor) para que `wpp.min.js` no bloquee el render sin alterar su orden en el head; se conserva la etiqueta original (id/data-*) al inyectar el atributo.
...
- WPP: además del filtro `script_loader_tag`, se usa `wp_script_add_data( 'wpp-js', 'strategy', 'defer' )` cuando está disponible para garantizar el `defer` del JS oficial sin romper el orden.
